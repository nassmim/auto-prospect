...
import { sendSmsAction } from "@/actions/messaging.actions";
...
export default function SmsTestSection({ accountId, hasApiKey }: SmsTestSectionProps) {
  ...
  return (
    <div className={`bg-white rounded-lg shadow-md p-8 ${!hasApiKey ? "opacity-50" : ""}`}>
      ...
      {hasApiKey ? (
        <div className="space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-black">Numéro destinataire</label>
            <input
              type="tel"
            ...
            />
            <p className="mt-1 text-xs text-black">Format international (ex: +33612345678)</p>
          </div>
          ...
          {sendResult && (
            ...
            >
              {sendResult.success ? "✓ SMS envoyé avec succès !" : `✗ Erreur: ${sendResult.error}`}
            </div>
          )}
        </div>
      ) : (
        <p className="text-sm text-black">Configure d&apos;abord ta clé API pour envoyer des SMS</p>
      )}
    </div>
  );
}
