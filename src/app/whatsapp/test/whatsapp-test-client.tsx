...
type WhatsAppTestClientProps = {
  initialData: InitialData;
};

export function WhatsAppTestClient({ initialData }: WhatsAppTestClientProps) {
  const [user, setUser] = useState(
    initialData ? { id: initialData.userId, email: initialData.email } : null,
  );
  ...
  const [connected, setConnected] = useState(initialData?.isConnected || false);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      window.location.reload(); // Reload to get server-side data
    }
  };
