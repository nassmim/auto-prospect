...
CREATE TABLE "organization_members" (
	...
	"organization_id" uuid NOT NULL,
	"name" text NOT NULL,
	CONSTRAINT "organisation_id_member_name_unique" UNIQUE("organization_id","name")
);
--> statement-breakpoint
...
ALTER TABLE "organization_members" ADD CONSTRAINT "organization_id_fk" FOREIGN KEY ("organization_id") REFERENCES "public"."organizations"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "organizations" ADD CONSTRAINT "auth_user_id_fk" FOREIGN KEY ("auth_user_id") REFERENCES "auth"."users"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE POLICY "enable all for organization owners" ON "organization_members" AS PERMISSIVE FOR ALL TO "authenticated" USING (exists (
        select 1 from organizations o
