...
CREATE POLICY "enable all for account members" ON "lead_reminders" AS PERMISSIVE FOR ALL TO "authenticated" USING (exists (
        ...
        where l.id = "lead_reminders"."lead_id"
        and l.account_id = (select auth.uid())));--> statement-breakpoint
CREATE POLICY "enable all for owners of the associated account" ON "leads" AS PERMISSIVE FOR ALL TO "authenticated" USING ("leads"."account_id" = (select auth.uid())) WITH CHECK ("leads"."account_id" = (select auth.uid()));--> statement-breakpoint
...
CREATE POLICY "enable all for account owners" ON "team_members" AS PERMISSIVE FOR ALL TO "authenticated" USING ("team_members"."account_id" = (select auth.uid())) WITH CHECK ("team_members"."account_id" = (select auth.uid()));